...
Following does not work because file does not exist.
...
```{python}
ndvi_stats_df = ndvi_naip_df(naip_stats_path, tract_cdc_gdf, chi_scenes_df)
```
...
```{python}
%store -r ndvi_stats_df
try:
    ndvi_stats_df
except NameError:
    ndvi_stats_df = ndvi_naip_df(naip_stats_path, tract_cdc_gdf, chi_tracts_df)
    %store ndvi_stats_df
```
