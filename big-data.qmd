...
# from landmapy.cdcplaces import hvplot_tract_gdf download_cdc_disease
# from landmapy.cdcplaces import join_tract_cdc plot_gdf_gv
%run cdcplaces.py
```
...
```{python}
# from landmapy.naip import naip_path, download_naip_tracks, ndvi_naip_df
%run naip.py
```
...
Following is local. Probably do not need.

```{python}
# from landmapy.check header_csv, import get_last_row_csv, check_element_in_csv
%run check.py
```

Code to check elements of CSV.

```{python}
header = header_csv(naip_index_path)
last_row = get_last_row_csv(naip_index_path)
print(header)
print(last_row)
check_element_in_csv(naip_index_path, header[0], last_row[0])
```

Following is duplicated in `naip.py`

```{python}
# check_naip_tracts(naip_index_path, chi_scenes_df)
```
...
```{python}
# from landmapy.explore import merge_ndvi_cdc, plot_ndvi_stats
%run explore.py
```

```{python}
ndvi_cdc_gdf = merge_ndvi_cdc(tract_cdc_gdf, ndvi_index_df)
model_fit = plot_ndvi_index(ndvi_cdc_gdf)
model_fit
```
...
```{python}
resid = plot_resid(logndvi_cdc_gdf, reg, yvar='asthma')
```

```{python}
(
    model_fit[0] + resid + model_fit[1]
)
```
