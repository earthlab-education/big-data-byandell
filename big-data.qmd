...
```{python}
from landmapy.habitat import robust_code, create_data_dir
```
...
```{python}
from landmapy.cdcplaces import shp_tract_path, download_census_tract, hvplot_tract_gdf
```
...
```{python}
from landmapy.cdcplaces import download_cdc_disease, join_tract_cdc, plot_gdf_gv
...
```{python}
from landmapy.explore import merge_ndvi_cdc, plot_ndvi_index
from landmapy.explore import var_trans, hvplot_matrix
from landmapy.explore import train_test, plot_train_test, plot_resid
```

```{python}
#| label: fig-modelfit
#| fig-cap: "Asthma and Edge Density Chloropleths"
